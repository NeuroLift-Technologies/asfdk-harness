...
### THREAD-014 — Windows-runtime fixes: a2a-task-watch registration + proxy spawn/CLI
| Field | Value |
|---|---|
| **Thread ID** | THREAD-014 |
| **Status** | 🟢 Complete |
| **Started** | 2026-09-20 |
| **Owner** | opencode |
| **Branch** | `feat/a2a-opencode-windows-fixes` |
| **Task** | Make governed A2A delivery work on the Windows host: fix the task-watch plugin's registration endpoint, fix the OpenCode proxy's spawn/CLI invocation, and verify end-to-end hub → proxy → OpenCode delivery. |
| **Scope** | `opencode-plugin/a2a-task-watch.ts`, `src/a2a-proxy.ts`, runtime services (discovery hub 3001, a2a proxy 4097), `docs/agent-log/`, this file |
| **Blockers** | Git index is broken on this Windows checkout (empty index; HEAD paths with `:` blocked by `core.protectNTFS`) — branch commit delivered via git plumbing; index repair escalated. |
| **Related PR** | [#33](https://github.com/NeuroLift-Technologies/asfdk-harness/pull/33) (`fix/win-safe-filenames` — index repair) · [#34](https://github.com/NeuroLift-Technologies/asfdk-harness/pull/34) (`feat/a2a-opencode-windows-fixes` — this thread) |
| **Notes** | (1) `a2a-task-watch.ts` targeted `POST {hub}/register`; hub only serves `/a2a/register` → corrected; live-verified (201, agent `opencode-asfdk-deploy` registered). (2) `a2a-proxy.ts` used `--attach <url>` (removed in OpenCode 2.0.10) and `--server <url>` (app server is auth-protected → 401 on its health check, incompatible) → spawn is now `opencode run --agent <agent> --format json` with the prompt on stdin, letting the CLI discover the running background service. (3) On Windows the PATH `opencode` is `.cmd`/`.sh` shims that `spawn()` cannot run → proxy started with `OPENCODE_BIN` = `C:\Users\joshd\AppData\Local\hermes\node\node_modules\@opencode\cli\bin\opencode.exe`. (4) Hub restarted with `ASFDK_HUB_STATE_FILE=~/.asfdk/discovery-hub-state.json` (persistence) and registry re-populated: `opencode`, `opencode-asfdk-deploy`, self-registered `a2a-opencode-proxy`. (5) E2E smoke: hub → proxy → OpenCode child spawn verified (task processing); completion depends on build-agent model call. (6) PR #34 review response (CodeRabbit): `OPENCODE_URL` is now honored when explicitly configured — the proxy passes `--server <url>` to `opencode run` (supported in 2.0.10) and falls back to CLI service discovery when unset; docstrings added to diff-touched functions in both files (docstring-coverage check). Full detail: `docs/agent-log/handoffs/2026-09-20-a2a-opencode-windows-fixes.json`. |
